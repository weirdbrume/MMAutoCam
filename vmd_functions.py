...
def create_cam_template_sequence(cam_templates, seq_len):
    ...
    Шаблон для последовательности выбирается случайным образом, один и тот же шаблон не должен
    повторяться два раза подряд, так же подряд не должены повторяться шаблоны из одной категории (дальней и
